...
int main(int argc, char *argv[])
{
    ...
    while ((choice = enter_choice()) != 13)
    {
        ...
        case 6:
            ...
            break;
        case 7:
            search_by_name(cf_ptr);
            break;
        case 8:
            sort_and_display(cf_ptr);
            break;
        case 9:
            csv_export(cf_ptr);
            break;
        ...
        case 11:
            ...
            break;
        case 12:
            transfer_between_accounts(cf_ptr);
            break;
        default:
            puts("Invalid choice. Enter 1-13.");
            break;
        }
    }
    ...
    while (fread(&client, sizeof(client_data_t), 1, fp) == 1)
    {
        if (strcmp(client.acct_num, acct_num) == 0)
        {
            if (out)
                *out = client;
            if (pos)
                *pos = ftell(fp) - (long)sizeof(client_data_t);
            return 1;
        }
    }
    return 0;
...
int save_record_at(FILE *fp, long pos, const client_data_t *rec)
{
    if (fseek(fp, pos, SEEK_SET) != 0)
        return -1;
    if (fwrite(rec, sizeof(client_data_t), 1, fp) != 1)
        return -1;
    fflush(fp);
    return 1;
}
...
void update_record(FILE *fp)
{
    ...
    case -1:
        ...
        return;
    case 0:
        printf("Account %s has no information.\n", acct);
        return;
    }
...
void transfer_between_accounts(FILE *fp)
{
    ...
    case -1:
        ...
        return;
    case 0:
        printf("Sender account %s not found.\n", from_acct);
        return;
    }
    ...
    case -1:
        ...
        return;
    case 0:
        printf("Receiver account %s not found.\n", to_acct);
        return;
    }
    ...
    puts("\nSender account:");
    print_header(stdout);
    print_record(stdout, &from_client);
    puts("\nReceiver account:");
    print_header(stdout);
    print_record(stdout, &to_client);
